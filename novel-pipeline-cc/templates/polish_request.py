...
def build_polish_request(
    ...
    Args:
        chapter_characters: 本章出场角色状态摘要
            格式示例: "林尘(主角/练气七层/焦躁/调查师傅失踪); 血煞门探子(敌对/练气五层/奉命侦查)"
...
if __name__ == "__main__":
    ...
    if len(sys.argv) >= 2:
        characters = sys.argv[1] if len(sys.argv) > 1 else "主角(练气七层)"
        ...
        sample = build_polish_request(
            chapter_characters="林尘(主角/练气七层/焦躁/调查师傅失踪); 血煞门探子(敌对/练气五层/奉命侦查)",
            draft_text="林尘在后山修炼。突然玉佩发光。一个黑衣人出现。两人打起来。林尘赢了。",
