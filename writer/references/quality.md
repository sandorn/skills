...
# fallback：仍有超标段时单独跑拆分
python scripts/split_paragraphs.py --batch chapters/
```

`pad_chapter.py` 自动完成：缺字章识别 → 全文稀疏段扩充 → 段落拆分（≤60字）。不需要再单独跑 `split_paragraphs.py`，除非 pad 后仍有少量超标段。
...
**注意：批量字数扩充时的段落陷阱。** 用 `echo >>` 或批处理追加文字来扩充字数时，追加内容通常会变成一行超长段落（数百汉字）。扩充完成后必须跑句号拆分——按 `。！？` 断段，确保每段≤60汉字：
...
输出逐章字数/违禁/段落超标状态。如果全部 PASS（字数≥2500 + 禁令清零 + 段落≤60），质检完成。
...
4. **扩充短章**：字数不足的章节用 `pad_chapter.py` 扩充：

   `pad_chapter.py` v7 扫描全文定位薄段，就地扩充感官细节+动作延展，不引入新剧情。扩充策略遵循五层叠料法：
   ...
   注意：叠料不改情节。所有新增内容必须落在已有场景内。

   `pad_chapter.py` 内建段落拆分，不需要额外运行 `split_paragraphs.py`。
