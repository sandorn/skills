...
## MCP 工具增强（质检全 Gate 可调用）

| Gate | MCP 工具 | 增强作用 |
|------|---------|---------|
| A 禁用词 / B 句式 | `publishready` | **语义级** AI 腔检测：套话句式、过度润色痕迹、罐头比喻 |
| C 心理外化 / E 对话 | `firstory` | 角色对话 OOC 检测、配角工具人化警告 |
| D 节奏 / F 结尾 | `uno` `analyze_text` | 叙事位置评估、情绪/语气评价、重复模式检测 |
...
| Gate | 检查内容 | 处理方式 |
|------|----------|----------|
| **A 禁用词** | AI 高频词替换 | **先调用 `publishready`** 语义扫描定位 AI 腔段落 → 再查禁止词表逐词替换 |
| **B 句式** | AI 惯用句式重写 | `publishready` 检测套话句式 → 打散排比/对称/空洞抒情 |
| **C 心理外化** | 情绪用动作展示 | `publishready` 检测情感直述密度 → 抽象词转身体动作 |
| **D 节奏** | 打断排比，长短交错 | `uno` `analyze_text` 评估叙事节奏 → 连续排比保留1-2个 |
| **E 对话** | 去 AI 腔，加口语化 | **先调用 `firstory`** 检查角色声音一致性 → 再逐句改口语化 |
| **F 结尾** | 去升华，用动作收尾 | `publishready` 检测结尾升华句式 → 改用具体动作/细节定格 |
