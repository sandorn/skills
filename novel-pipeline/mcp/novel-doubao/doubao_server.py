    ...
    try:
        async with httpx.AsyncClient(timeout=300.0) as client:
            resp = await client.post(
                f"{DOUBAO_BASE_URL}" if DOUBAO_BASE_URL.endswith("chat/completions") else f"{DOUBAO_BASE_URL}/chat/completions",
                headers={
            ...
            if finish_reason == "length":
                return f"ERROR_TRUNCATED: max_tokens limit hit. Content length={len(content)}. Partial content:\n---\n{content}"
            return content
