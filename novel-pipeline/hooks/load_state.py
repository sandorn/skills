...
触发: Claude 会话启动，novel-pipeline skill 激活时

扫描 state-files/ 目录，输出摘要供 Claude CLI 加载为会话上下文。
"""
import sys, json, os
...
def _summarize(fname: str, data: dict) -> tuple[int, str]:
    ...
    if fname == "world_setting.json":
        ...
        return n, f"[世界观] {len(factions)}个势力({', '.join(names[:5])}), {len(realms)}个境界"
        ...
        for c in chars[:10]:
            ...
            summaries.append(f"{name}({role}{'/'+cult if cult else ''})")
        ...
        return len(realms) + len(equip) + len(rules), f"[战力] {len(realms)}境界, {len(equip)}装备, {len(rules)}战斗规则"
